...
async function loadOrders() {
  ...
  ordersContainer.innerHTML =
    '<p id="loading" style="text-align:center; padding:20px; font-family:\'Poppins\',sans-serif;">Loading your orders...</p>'
  try {
    const database = await initFirebase()
      ...
      (snapshot) => {
        ...
        myOrders.forEach((order) => {
          ...
          const itemsList = order.items
            ? order.items
                .map((i) => `<div>${i.name} × ${i.qty} <span>₹${(i.price * i.qty).toFixed(2)}</span></div>`)
                .join("")
            : "<div>No items</div>"
...
// ✅ FIXED: renderMapEmbed — uses Embed API (no ref error)
function renderMapEmbed(lat, lng, zoom = 15) {
  const mapEl = document.getElementById("track-map")
  if (!mapEl) return

  const { getGoogleMapsKey } = window._configModule || {}
  let apiKey = "AIzaSyCPbOZwAZEMiC1LSDSgnSEPmSxQ7-pR2oQ"

  import("./config.js")
    .then((mod) => {
      apiKey = mod.getGoogleMapsKey()
      const url = `https://www.google.com/maps/embed/v1/place?key=${apiKey}&q=${lat},${lng}`
      mapEl.src = url
    })
    .catch(() => {
      const url = `https://www.google.com/maps/embed/v1/place?key=${apiKey}&q=${lat},${lng}`
...
document.addEventListener("click", async (e) => {
  ...
  trackPopup.onclick = (ev) => {
    if (ev.target === trackPopup) closeTracking()
  }
})
